...
def download_img(flag):
    ...
    with open(f"cia_a_flags/{flag}.jpg", "wb") as img_f:
        img_f.write(img)
    return len(img)
...
def main():
    ...
    if not os.path.exists("cia_a_flags"):
        os.makedirs("cia_a_flags")
    ...
    with open("cia_a_result.txt", "w", encoding="utf-8") as result_f:
